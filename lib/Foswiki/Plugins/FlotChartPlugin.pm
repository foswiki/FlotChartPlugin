...
sub doInit {
    ...
    Foswiki::Func::addToZone( 'script', $pluginJavascript, $jscript,
        'JQUERYPLUGIN' );
...
sub _FLOTCHART {
    ...
    my @xAxisOptions = ();
    $val = $params->{xticks};
    push @xAxisOptions, "ticks: [$val ]" if defined $val;
    $val = $params->{xmin};
    push @xAxisOptions, "min: $val" if defined $val;
    $val = $params->{xmax};
    push @xAxisOptions, "max: $val" if defined $val;
    $val = $params->{xmode};
    push @xAxisOptions, "mode: '$val'" if defined $val;
    $val = $params->{xmargin};
    push @xAxisOptions, "autoscaleMargin: $val" if defined $val;
    ...
    my @yAxisOptions = ();
    $val = $params->{yticks};
    push @yAxisOptions, "ticks: [$val]" if defined $val;
    $val = $params->{ymin};
    push @yAxisOptions, "min: $val" if defined $val;
    $val = $params->{ymax};
    push @yAxisOptions, "max: $val" if defined $val;
    $val = $params->{ymode};
    push @yAxisOptions, "mode: '$val'" if defined $val;
    $val = $params->{ymargin};
    push @yAxisOptions, "autoscaleMargin: $val" if defined $val;

    push @plotOptions, "yaxis : {" . join( ", ", @yAxisOptions ) . "}"
      if @yAxisOptions;

    my $options;
    $options = getOptions( $params, 'points' );
    push @plotOptions, $options if $options;
    $options = getOptions( $params, 'lines' );
    push @plotOptions, $options if $options;
    $options = getOptions( $params, 'bars' );
    push @plotOptions, $options if $options;
    ...
    foreach my $name ( keys(%seriesData) ) {
        ...
        my $options;
        $options = getOptions( $params, 'points', $name );
        push @seriesOptions, $options if $options;
        $options = getOptions( $params, 'lines', $name );
        push @seriesOptions, $options if $options;
        $options = getOptions( $params, 'bars', $name );
        push @seriesOptions, $options if $options;
...
EOS
    ...
    Foswiki::Func::addToZone( 'script', "!$pluginName - flotChart $ChartId",
        $result, $pluginJavascript );
...
sub getOptions {
    ...
    $val = $params->{"${name}$sep${type}"};
    push @options, "show: $val" if defined $val;
    $val = $params->{"${name}$sep${type}width"};
    push @options, "lineWidth: $val" if defined $val;
    $val = $params->{"${name}$sep${type}fill"};
    push @options, "fill: $val" if defined $val;
    $val = $params->{"${name}$sep${type}fillcolor"};
    push @options, "fillColor: $val" if defined $val;
    $val = $params->{"${name}$sep${type}shadow"};
    push @options, "shadowSize: $val" if defined $val;

    if ( $type eq 'points' ) {
        $val = $params->{"${name}$sep${type}radius"};
        push @options, "radius: $val" if defined $val;
    }

    if ( $type eq 'bars' ) {
        $val = $params->{"${name}$sep${type}width"};
        push @options, "barWidth: $val" if defined $val;
    }
